...
def getColumn():
    columns = {}
    url = 'http://172.28.3.157:8080/xchzxt/cenrep/queryByIdGridPanel.action?id=1301'
...
def getData():
    ...
    start = 0
    limit = 6000
    url = 'http://172.28.3.157:8080/xchzxt/Dp/queryByCondFormclxx.action'
    data = {'start': start, 'limit': limit, 'name': '', 'jd_code': '', 'sq_code': '', 'isExact': 0}
    ...
    if len(infos) > 0:
        ...
        if pageCount > 1:
            for i in range(1, pageCount + 1):
                start = i * limit
                data = {'start': start, 'limit': limit, 'name': '', 'cjzh': '', 'cjdj': '', 'sex': '', 'hjd': '', 'isExact': 0}
                result = json.loads(requests.post(url, data).content.decode())
                infos = result['root']
                if len(infos) > 0:
                    for info in infos:
