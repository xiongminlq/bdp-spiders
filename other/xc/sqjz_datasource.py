...
def getColumn():
    columnss = {}
    #系统列表展示字段信息的url
    url = 'http://172.28.3.157:8080/xchzz/cenrep/queryByIdGridPanel.action?id=19'
...
def getData():
    ...
    with open('data\\社区矫正人员_数据中心.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=values)
        writer.writeheader()
        start = 0
        limit = 1000
        #列表详情字段信息的url,包含了列表展示数据的总数
        url = 'http://172.28.3.157:8080/xchzz/gx/GX_WT_CommRectify/queryByCondFormWTCommRectify.action'
        ...
        if len(infos) > 0:
            ...
            #矫正类别
            rectifyType = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=GX_DIC_RECTIFYTYPE&_dc=1560479366768')
            #活动地区类型
