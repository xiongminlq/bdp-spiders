...
def getColumn():
    columnss = {}
    #系统列表展示字段信息的url
    url = 'http://172.28.3.157:8080/xchzz/cenrep/queryByIdGridPanel.action?id=580'
...
def getData():
    ...
    with open('data\\艾滋病人.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=values)
        writer.writeheader()
        start = 0
        limit = 1000
        #列表详情字段信息的url,包含了列表展示数据的总数
        url = 'http://172.28.3.157:8080/xchzz/peo/queryByCondFormPersonaids.action'
        ...
        if len(infos) > 0:
            ...
            #宗教信仰
            religionType = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=STD_GB_RELIGIONTYPE&_dc=1560476217894')
            #感染途径
            infection = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=ZZ_DIC_INFECTION&_dc=1560476250460')
            #犯罪类型11
            crimetype = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=ZZ_DIC_ORIGINALCHARGE&_dc=1560476250464')
            #关注类型
            focustype = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=ZZ_DIC_FOCUSTYPE&_dc=1560476250466')
            #收治情况
            curecase = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=ZZ_DIC_CURECASE&_dc=1560476250469')
            #收治机构所属层级
            cureoraLevel = getDict('http://172.28.3.157:8080/xchzz/cenrep/common/loadGeneralCodeCodeLoader.action?codeTableName=ZZ_DIC_CURECASETIER&_dc=1560476250473')
            ...
            for info in infos:
                ...
                for item in info:
                    if item in columns:
                        ...
                        if item == 'infection' and info[item] in infection:
                            info[item] = infection[info[item]]
                        if item == 'crimetype' and info[item] in crimetype:
                            info[item] = crimetype[info[item]]
                        if item == 'focustype' and info[item] in focustype:
                            info[item] = focustype[info[item]]
                        if item == 'curecase' and info[item] in curecase:
                            info[item] = curecase[info[item]]
                        if item == 'cureoraLevel' and info[item] in cureoraLevel:
                            info[item] = cureoraLevel[info[item]]
